...
useSeoMeta({ title: 'Revisar Posts — seConecta' })
...
async function fetchPosts(reset = true) {
  ...
  } catch { error.value = 'Não foi possível carregar os posts.' }
  finally { loading.value = loadingMore.value = false }
}
...
function setTag(tag: string) {
  ...
  feedMode.value = 'general'; searchQuery.value = ''; fetchPosts(true)
}
function setFeedMode(mode: string) {
  feedMode.value = mode; activeTag.value = ''; searchQuery.value = ''; fetchPosts(true)
}

watch(searchQuery, debounce(() => {
  if (feedMode.value === 'personalized') feedMode.value = 'general'
  activeTag.value = ''; fetchPosts(true)
}, 400))
  ...
  <div class="min-h-screen">
    ...
    <section class="relative overflow-hidden py-16 px-8 bg-none" aria-labelledby="hero-heading">
      ...
      <div class="relative z-10 max-w-[900px] mx-auto w-full">
        ...
        <p data-aos="fade-up" class="font-medium text-[#079272] max-w-[440px] leading-relaxed mb-8 text-base">
      ...
      <div class="max-w-[900px] mx-auto px-4 md:px-8 pt-10 pb-4">
        <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
          ...
          <div v-if="isAuthenticated && hasInterests" class="flex gap-1 bg-[#f7f5f0] border border-[#e8e4dc] rounded-xl p-1">
            <button v-for="m in [{ key: 'personalized', label: 'Para você' }, { key: 'general', label: 'Explorar' }]" :key="m.key"
              ...
              @click="setFeedMode(m.key)">{{ m.label }}</button>
          </div>
        </div>

        <div class="relative mb-4">
          <svg class="absolute left-4 top-1/2 -translate-y-1/2 pointer-events-none text-gray-400" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
          ...
          <svg v-if="loading && searchQuery" class="absolute right-4 top-1/2 -translate-y-1/2 animate-spin text-[#079272]" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
        </div>
      ...
      <main class="max-w-[900px] mx-auto px-4 md:px-8 pb-24 pt-2 flex flex-col gap-4">
        <template v-if="loading">
          <div v-for="i in 4" :key="i" class="bg-white rounded-2xl border border-[#e8e4dc] p-8 animate-pulse flex flex-col gap-3">
            <div class="flex gap-3"><div class="h-3 w-20 bg-[#f0ece5] rounded-full"></div><div class="h-3 w-16 bg-[#f0ece5] rounded-full"></div></div>
            ...
            <div class="flex justify-between mt-1"><div class="flex gap-2"><div class="h-8 w-16 bg-[#f0ece5] rounded-lg"></div><div class="h-8 w-8 bg-[#f0ece5] rounded-lg"></div></div><div class="h-8 w-20 bg-[#f0ece5] rounded-lg"></div></div>
          </div>
        </template>
        ...
        <div v-else-if="posts.length === 0" class="flex flex-col items-center py-20 gap-3 text-center">
          ...
          <button v-if="activeTag || searchQuery" class="text-xs text-[#079272] underline border-none bg-transparent cursor-pointer" @click="setFeedMode('general'); activeTag = ''; searchQuery = ''">Ver todos</button>
        </div>

        <template v-else>
          <FeedPostCard v-for="(post, index) in posts" :key="post.id" :post="post" :index="index" @read="openArticle" />
          <div v-if="hasMore" class="flex justify-center pt-2">
            <button class="flex items-center gap-2 text-sm font-semibold px-8 py-3 border border-[#e8e4dc] text-[#666] rounded-xl hover:border-[#0d0d0d] hover:text-[#111] transition-all disabled:opacity-40 cursor-pointer bg-transparent"
